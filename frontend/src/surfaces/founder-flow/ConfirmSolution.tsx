/**
 * The last look at the solution — Founder Flow v2, the reference's `probConfirm`
 * at `vStep` 1.
 *
 * BUILT 2026-08-20 from the supplied reference (`Proovd Founder Flow v2.dc.html`,
 * `[data-pconfirm]` / `kindWide`), by explicit product direction, and inserted
 * where the reference puts it: between the last look at the problem and
 * Positioning. `stillMine` on the problem runs `afterSection({vStep:
 * st.vStep+1})`, `isProbConfirm` is `n==='vetting' && !vReviewing && vStep<2`,
 * so `vStep` 1 is still this composition — and `vStep` 2 is the first of the
 * questions that build on the pair, which is §9's third answer.
 *
 * ── One component there, two pages here ────────────────────────────────────
 * Every difference between the two is a ternary on `vStep` in the reference's
 * own view model, and this file carries all four:
 *
 *     pcHead:st.vStep===1?'You said this is Teeb’s solution, gotta make sure.'
 *                        :'You confirmed this was the problem, just double checking.',
 *     pcText:st.vStep===1?st.solText:st.probText,
 *     pcCta:st.pcEditing?'Save':(st.vStep===1?'Still my solution':'Still my problem'),
 *     setProbText:e=>this.setState(st.vStep===1?{solText:…}:{probText:…}),
 *
 * The layout, the heights, the relay, the toggle, the chrome and the Back
 * control are the problem's, because in the reference they are literally the
 * same markup. They are reproduced here rather than shared with
 * `ConfirmProblem` for the reason the register records: a page in this product
 * is an ADDRESS, the two pages are two addresses, and a shared component
 * parameterised by which answer it is showing would put the flow's forward and
 * back targets behind a prop rather than in the router. The two do share the
 * one thing that must never diverge — `PHASE 54`'s `.ff-pc` block, which IS
 * the reference's `[data-pconfirm]` stylesheet.
 *
 * ── This is not `ConfirmAnswer` either ─────────────────────────────────────
 * `ConfirmAnswer` is `[data-problem]` — screens 2 and 3, where the answer is
 * first read and edited, on a 1304px column with a sticker in the headline, a
 * scroll rail, a `Next` CTA and a `1.38` stage boost. This is a 1760px column
 * pushed 70px down, no sticker, no rail, no boost, a different headline and a
 * different CTA. They share one RECORD and nothing else, which is the
 * reference's own arrangement: `pcText` reads `st.solText`, the same state
 * `[data-problem]` writes on screen 3.
 *
 ...
 * `vReviewing` is false, so its `s *= .9` Last-look branch is off — so it takes
 * `pageScale` alone. Inside the stage sits a 1760px column, centred and pushed
 * 70px down, and every child below carries the reference's own pixel value.
 * Nothing reflows; the stage scales instead.
 *
 ...
 * are almost the same total height and why the composition does not jump.
 * Reproducing it as a collapsed-to-zero button instead would leave a 0px
 * control in the tab order and would animate a height the reference does not.
 *
 ...
 * button that would carry it is not rendered. Not implemented here, and that is
 * the reference's own behaviour rather than an omission.
 *
 * ── The product name is the real one ───────────────────────────────────────
 * The reference hardcodes `Teeb`. The sentence structure is kept exactly and
 * the value comes from `fetchDraftLanding`, which is the read the invite page
 * and the reach orbit already use for it. A draft with no product name
 * recorded renders `your solution` rather than an empty possessive — the
 * headline is the one string on this screen that cannot degrade to a blank
 * (§33.11.3 forbids the placeholder, and a bare `’s solution` is worse than
 * either).
 *
 ...
 * `ConfirmProblem` and `ConfirmAnswer` already record — the copy is the
 * specification here. `Still my solution` is not objectless and needs none.
 *
 ...
 * for this page exactly as it is for the problem: left in place it would sit
...
import { fetchDraftLanding } from '../../features/admin/api.js';
...
/** `pcCta`, for `vStep` 1. Verbatim. */
const CTA_LABEL = 'Still my solution';

/**
 * `pcHead`, for `vStep` 1. Its sentence, with its own curly apostrophe, and the
 * product name substituted where it hardcodes `Teeb`.
 */
function headline(productName: string): string {
  const name = productName.trim();
  return name
    ? `You said this is ${name}’s solution, gotta make sure.`
    : 'You said this is your solution, gotta make sure.';
}
...
export function ConfirmSolution() {
  ...
  useEffect(() => {
    let cancelled = false;
    // Both reads at once: the headline needs the product name and the field
    // needs the answer, and neither is worth a second wait.
    Promise.all([fetchVetting(token), fetchDraftLanding(token)])
      .then(([vetting, draft]) => {
        if (!cancelled) setLoaded({ vetting, product: draft.productName });
      })
      .catch(() => {
  ...
  return (
    <FlowPage pageId="confirm-solution" param={token}>
      <ConfirmScreen
        token={token}
        initial={loaded.vetting.solution ?? ''}
...
function ConfirmScreen({
  ...
  function update(next: string) {
    setValue(next);
    autosave.queue({ solution: next });
  }

  /**
   * `toggleProbEdit`. Its `pcToggle` saves the answer whenever it LEAVES the
   * editor (`if(!on){ … this.setAns(sol?'solution':'problem', …) }`); the
   * autosave has already queued every keystroke, so what that means here is
  ...
  /**
   * `stillMine`, at `vStep` 1 — `setAns('solution', …)` then
   * `afterSection({vStep: st.vStep+1})`, which is an ordinary forward step to
   * the first of the questions that build on the pair. Its editing branch —
   * "Save just closes the editor" — is unreachable, because the button
   * carrying it is not rendered while editing.
   */
  async function stillMine() {
    if (busy) return;
    setBusy(true);
    try {
      // Land what is typed before moving. Positioning reads it back, and its
      // own submit gate refuses while it is empty.
  ...
  return (
    <div className="ff-pc" ref={root}>
      {/* The reference's own control, bottom-left. `back()` at `vStep` 1 is
          `this.step({vStep: st.vStep-1}, 'back')` — the last look at the
          problem, with the back relay. Its label names where it goes only to a
          screen reader: the visible word is its own `Back`, and §33.11.4's
          objectless-CTA rule is answered by the accessible name rather than by
          overriding the reference's copy. */}
      <button
        type="button"
        className="ff-pc__back"
        aria-label="Back to Confirm the basics"
        onClick={() => leave(founderFlowPath('confirm-problem', token), -1)}
      >
        <svg
          viewBox="0 0 24 24"
          width="11"
        ...
        <div className="ff-pc__col">
          ...
          <div className="ff-pc__panel" data-stage-anim="panel">
            <textarea
              ...
              aria-label="The solution you described"
              readOnly={!editing}
              value={value}
