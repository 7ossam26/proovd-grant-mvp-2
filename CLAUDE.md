...
**Phases 00–08 are complete, and Phase 09 is built in halves — 09a is done, 09b is next.** Phase 06 was built in two halves — 06a the configuration surface, 06b the invitation — Phase 08 in three: 08a recruitment, 08b the compact signup, 08c the preparing reveal. §33.1.1 through §33.1.9 all pass, so do §33.2.1 through §33.2.4, and so do §33.3.1 through §33.3.4. **Phase 09b is next** — see "What Phase 09 owns" below and `docs/master-plan.md` §6. Every path this file names exists at the path it names. `docs/phases/phase-00.md` … `phase-24.md` are all present.
...
- **No R2 bucket is configured, so uploads refuse loudly, and the four §6 interview settings are unset, so nothing is bookable.** Both are Track A4. `unconfiguredStorage` throws exactly as `unconfiguredTransport` does, and the workspace renders no upload control and no booking control rather than one that would fail (§1.4). Do not stub either.
...
| **09b** | Cal.com Cloud embed, webhook ingestion into the booking record, the reconciliation job, and §27.3's four interview notifications (`founder_interview_confirmed`, `_reminder`, `_rescheduled`, `_canceled`) | — (its rules are already tested through 09a's record) | next |

**The booking record is 09a's, not 09b's, and the split runs there for a reason.** tech-stack §12: "The booking record in our database is the source of truth, populated from Cal.com webhooks." So the domain state, its four §12 conditions, its reschedule history, and the §33.3.3 recalculation are all built and tested here; 09b adds a vendor that *feeds* them. Phase 09's own trap — "Cal.com is a source of events, not truth… don't leave `confirmed` reachable only by webhook" — is satisfied by 09a shipping the reconciliation path first. The four notification keys are deliberately **not** in `notifications/events.ts` yet: a key with no sender claims a message the product does not send (§1.4).
