...
| **12b** | §14.4 campaign building, §15 roster readiness, review readiness and preview, Admin review, materiality and reacceptance | §33.3.9, §33.3.10, §33.4.1, §33.4.2 | **next** |

**12a stopped before anything §15 owns.** `affiliate_roster_status` still only ever becomes `failed` (the §14.6 outcome); `launch_ready` is 12b's decision, `campaign_build_status` moves only from Phase 11's `in_progress` stamp, and `review_ready` stays derived and false. The materiality machinery, the build surface, and the preview do not exist yet — not stubbed, absent.
