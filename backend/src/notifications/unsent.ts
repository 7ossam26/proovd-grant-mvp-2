...
export interface UnsentEvent {
   ...
   *
   * Only `none` remains. Every phase-owned absence was closed by the phase
   * that owned it — which is the whole reason the owner field exists rather
   * than a flat "not built yet".
   */
  owner: 'none';
  reason: string;
...
export const UNSENT_NOTIFICATION_EVENTS = {
  ...
  /*
   * Nothing else. Phase 22b closed all 44 `message` absences and Phase 21b
   * built the five `capability` ones, so every §27 key whose behaviour exists
   * has a sender and the only entries left are the three the Spec itself rules
   * out. That is the register doing exactly what it was built for: the list
   * shrank to the genuinely undecidable rather than to the conveniently
   * ignored, and it did so by the phases that owned each gap closing it.
   */
